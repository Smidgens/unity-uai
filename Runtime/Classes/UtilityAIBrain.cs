...
namespace Smidgenomics.Unity.UAI
{
	...
	public sealed class UtilityAIBrain
	{
		...
		public UtilityAIContext GetContext() => _context;

		public static UtilityAIBrain CreateBrain(in UtilityBrainInitConfig config)
		{
			var brain = new UtilityAIBrain();
			brain._contextGameobject = config.gameObject;
			brain._behaviour = config.behaviourDefinition;
			brain._bucketSelector = brain._behaviour._bucketSelector ?? UtilityManager.DefaultBucketSelector;
			return brain;
		}
		...
		public UtilityAISelector GetCurrentBucketSelector()
		{
			return _bucketSelector;
		}

		public UtilityAISelector GetCurrentActionSelector()
		{
			return IsValidBucketID(_currentBucketID)
			? _bucketRecords[_currentBucketID].actionSelector
			: UtilityManager.DefaultActionSelector;
		}
		...
		public void StartLogic()
		{
			...
			_context = new UtilityAIContext
			{
				gameObject = _contextGameobject,
				memory = _memory
			};
		...
		private UtilityAIContext _context = default;
		...
		private UtilityAISelector _bucketSelector = new UtilityAISelector_TopScore();
		private GameObject _contextGameobject = null;
		private UtilityAIMemory _memory = null;
		private UtilityAIBehaviour _behaviour = null;

		private UtilityAIBrain()
		{
			_memory = new UtilityAIMemory();
		}
		...
		internal struct ActionRecord
		{
			...
			public UtilityAIAction template;
			public UtilityAIAction instance;
			...
			public EUtilityAIActionStatus Status => instance != null ? instance.GetActionStatus() : EUtilityAIActionStatus.Inactive;
		}
		...
		internal struct BucketRecord
		{
			...
			public UtilityAIBucket bucketSO;
		...
		private void InitExecutionContext()
		{
			...
			foreach(var bucketConfig in _behaviour._buckets)
			{
				...
				bucketRecord.considerations = bucketConfig.overrideConsiderations
				? bucketConfig.considerations._considerations.GetItems()
				: bucketConfig.bucket._bucketConsiderations.GetItems();

				bucketRecord.weight = bucketConfig.overrideWeight
				? bucketConfig.weight
				: bucketConfig.bucket._weight;

				bucketRecord.actionSelector = bucketConfig.overrideSelector && bucketConfig.actionSelector != null
				? bucketConfig.actionSelector
				: bucketConfig.bucket._actionSelector;
				bucketRecord.actionSelector = bucketRecord.actionSelector ?? UtilityManager.DefaultActionSelector;
		...
		private int SelectBucket()
		{
			...
			{
				return _actionRecords[_bucketIndicesByScore[i]].score;
			});
		...
		void CancelAction(int actionID, Action onDone)
		{
			...
			DeactivateAction(actionID, EUtilityAIActionStatus.Cancelled, onDone);
		}

		private void DeactivateAction(int actionID, EUtilityAIActionStatus status, Action onDone)
		{
			if (_deactivatingAction)
			{
				return;
		...
		private void ActivateAction(int actionID)
		{
			...
			record.instance._status = EUtilityAIActionStatus.Active;
		...
		{
			DeactivateAction(_currentActionID, EUtilityAIActionStatus.Finished, ResetAction);
		}
